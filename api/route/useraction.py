...
def login():
    try:
        ...
        print(device)
        print(browser)
        newuser = User(ip=ip,os=os,browser=browser,device=device,page='',tag='TagB')
        newuser.save()
...
def get_user_info_one():
    ...
    try:
        ...
        temp = RequestData.objects(user=ObjectId(id)).skip( int(offset) ).limit( int(items_per_page) )
        list = []
        for val in temp:
            k = user
            k.page = val.targetURL
            list.append(k)
        total = int(int(RequestData.objects(user=ObjectId(id)).count()) / int(items_per_page))
        return successResponseWrap({'list': list, 'total': total})
