...
class UserMaterial():
    ...
    def generate_user_data(self, amount=50):
        """Generate mock user data with specific amount
        ...
        """
        user_ips = ip = [fake.ipv4() for _ in range(int(amount))]
        user_list = []
        for ip in user_ips:
            user_info = {}
...
class RequestMaterial():
    ...
    def generate_request_data(self, users):
        """Generate mock request data triggered by some users

        Args:
            users (list | Optional): the list of users that the requests are generated by
            if users params not provided, user data will be treated as empty dict and generate 50 reqeusts data
